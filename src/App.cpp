...
void ff::App::createInstance() {
    try {
        ...
        // Ñîçäàíèå èíñòàíñà Vulkan
        instance = vk::createInstance(instanceCreateInfo);

        // Ñîçäàíèå èíñòàíñà Vulkan
        instance = vk::createInstance(instanceCreateInfo);
    } catch (const std::exception &ex) {
        std::cerr << "Îøèáêà ñîçäàíèÿ Vulkan Instance: " << ex.what() << std::endl;
    }
}
...
void ff::App::createImageViews() {
    ...
    for (const auto &swapchainImage: swapchainImages) {
        ...
        imageViewCreateInfo.setViewType(vk::ImageViewType::e2D);// èçîáðàæåíèå ðàññìàòðèâàåòñÿ êàê 2D-òåêñòóðà
        ...
        // Ñîçäàíèå ïðåäñòàâëåíèÿ èçîáðàæåíèÿ
        try {
...
void ff::App::createRenderPass() {
    // Ïîëó÷àåì ïîääåðæèâàåìûé ôîðìàò ãëóáèíû
    ...
    // 2. Ãëóáèííîå âëîæåíèå (ñ êîððåêòíûì ôîðìàòîì!)
...
void ff::App::drawFrame() {
    // Æä¸ì çàâåðøåíèÿ ïðåäûäóùåãî êàäðà
    ...
    // Ïîëó÷àåì èçîáðàæåíèå èç öåïî÷êè ñâîï÷åéíà
    ...
    imageInfo.setDeviceMask(1);// âñ¸ ðàâíî íóæåí, òàê êàê èñïîëüçóåì acquireNextImage2KHR

    auto [result, imageIndex] = device.acquireNextImage2KHR(imageInfo);

    if (result != vk::Result::eSuccess && result != vk::Result::eSuboptimalKHR) {
        throw std::runtime_error("Íå óäàëîñü ïîëó÷èòü èçîáðàæåíèå èç swapchain.");
    }

    // Ñáðîñ è çàïèñü êîìàíä
    commandBuffers[imageIndex].reset();
    writeDataIntoCommandBuffers(imageIndex);

    // Íàñòðîéêà îáû÷íîãî submit ñ vk::SubmitInfo
    ...
    if (presentResult != vk::Result::eSuccess && presentResult != vk::Result::eSuboptimalKHR) {
        throw std::runtime_error("Íå óäàëîñü ïðåäñòàâèòü èçîáðàæåíèå.");
    }
}
